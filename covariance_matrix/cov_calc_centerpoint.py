...
def normalize_angle_diff(diff):
    """
    Normalize angle difference to [-π, π] range to handle flips between π and -π
    """
    while diff > np.pi:
        diff -= 2 * np.pi
    while diff < -np.pi:
    ...
    for scene_token in tracks_gt.keys():
        ...
        for t_idx in range(len(tracks_gt[scene_token].keys())):
            ...
            for tracking_name in NUSCENES_TRACKING_NAMES:
                ...
                for i, item in enumerate(pred_boxes[sample_token]):
                    for dets_outputs in item[tracking_name]:
                        det_all.append(dets_outputs['box'])

                if len(det_all) == 0:
                    continue
                
                # ours : x,y,z, w, l, h, rot or l, w
                ...
                matched_indices = list(zip(row, col))
                ...
                for pair_id in range(matched_indices.shape[0]):
                    if distance_matrix[matched_indices[pair_id][0]][matched_indices[pair_id][1]] < threshold:
                        det_box = dets[matched_indices[pair_id][0]]
                        gt_box = gts[matched_indices[pair_id][1]]
                        
                        # Calculate differences with special handling for rotation
                        diff_value = det_box - gt_box
                        diff_value[6] = normalize_angle_diff(det_box[6] - gt_box[6])  # Normalize rotation difference
                        ...
                        # Velocity calculation with normalized rotation
                        if t_idx > 0 and t_idx - 1 in match_diff_t_map[tracking_name] and gt_track_id in \
                                match_diff_t_map[tracking_name][t_idx - 1]:
                            prev_diff = match_diff_t_map[tracking_name][t_idx - 1][gt_track_id]
                            diff_vel_value = diff_value - prev_diff
                            # Normalize rotation velocity
                            diff_vel_value[6] = normalize_angle_diff(diff_value[6] - prev_diff[6])
...
if __name__ == '__main__':
    ...
    # split_scenes_tokens_set = set(split_scenes_tokens)
    # all_scene_tokens = {nusc.get('sample', sample)['scene_token'] for sample in pred_boxes.keys()}
    # all_scene_tokens_set = set(all_scene_tokens)
    # missing_tokens = split_scenes_tokens_set - all_scene_tokens
    # print('idk man', missing_tokens)
