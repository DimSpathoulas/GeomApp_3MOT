...
def distance_matrix_gen(d_t_map, mah_metric, dets, curr_gts, trks, prev_gts, state, DCS1):
    ...
    if state < 40 and prev_gts.shape[0] != 0 and curr_gts.shape[0] != 0 and D_mod.shape[0] > 0:
...
def track_nuscenes():
    ...
    parser.add_argument('--output_path', type=str, default='g2_lr0001_ep40.json',
    ...
    writer = SummaryWriter('runs/g2_lr0001_ep40')

    for epoch in range(EPOCHS):

        total_loss = torch.tensor(0.0).to(device=device)
