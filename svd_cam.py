...
def svd():
    ...
    for name in NUSCENES_TRACKING_NAMES:
        if len(fvs_all[name]) == 0:
            continue

        # Stack and flatten the features (shape: (m, 1024))
        fvs = np.vstack(fvs_all[name])

        # Center the data and normalize them
        mean_vector = np.mean(fvs, axis=0)
        std_vector = np.std(fvs, axis=0)
        std_vector[std_vector == 0] = 1e-8  # Avoid division by zero

        fvs = ( fvs - mean_vector ) / std_vector # mean shift and normalize

        # Convert to tensor
        fvs_tensor = torch.tensor(fvs, dtype=torch.float32)

        # Run SVD
        print(f"Running SVD for {name}...")
        U, S, Vh = torch.linalg.svd(fvs_tensor, full_matrices=False)
