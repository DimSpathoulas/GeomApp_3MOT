...
def distance_matrix_gen(d_t_map, mah_metric, dets, curr_gts, trks, prev_gts, state, DCS1):
    ...
    if state < 10 and prev_gts.shape[0] != 0 and curr_gts.shape[0] != 0 and D_mod.shape[0] > 0:
        ...
        K = torch.randint(0, 2, size=D_mod.shape, device=device).float()
...
def track_nuscenes():
    ...
    parser.add_argument('--output_path', type=str, default='g2_output.json',
    ...
    writer = SummaryWriter('runs/g2_main_rand')
