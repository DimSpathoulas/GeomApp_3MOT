...
from Nets.dist_comb_net import DIST_COMB_MODULE
...
class AB3DMOT(object):
    def __init__(self, max_age=2, min_hits=3, tracking_name='car', state=0, COMB=None):
        """
        observation:
                          [0, 1, 2, 3, 4, 5, 6]
    ...
    def update(self, dets_all):
        ...
        D_mah = torch.tensor(mahalanobis_distance(dets=dets, trks=trks, trks_S=trks_S)).to(device)
        st = 0
        D_module, det_feats = self.COMB(F2D, F3D, cam_onehot_vector, trks_feats, D_mah, st)

        K = None
        if self.state < 10 and prev_gts.shape[0] != 0 and curr_gts.shape[0] != 0 and D_module.shape[0] > 0:
            K = construct_K_matrix_remake(distance_matrix=D_module, dets=dets, curr_gts=curr_gts, trks=trks, prev_gts=prev_gts)
            
        D = D_module.detach().cpu().numpy()
        
        mah = D_mah.detach().cpu().numpy()
        D = D + mah
        ...
        matched, unmatched_dets, unmatched_trks = associate_detections_to_trackers(matched_indices=matched_indexes,
                                                                                   distance_matrix=D,
                                                                                   dets=dets, trks=trks, mahalanobis_threshold=self.mahanalobis_thresh)
...
def track_nuscenes():
    ...
    parser.add_argument('--dets_val', type=str,
                        ...
                        help='Path to detections, train split for train - val split for inference')
    parser.add_argument('--model_state', type=str, default='model.pth',
                        help='destination and name for model')
    parser.add_argument('--output_path', type=str, default='DIST_COMB.json',
    ...
    # params_to_optimize = list(COMB.G1.parameters()) + list(COMB.inter.parameters()) + list(COMB.G2.parameters())
    params_to_optimize = list(COMB.G1.parameters()) + list(COMB.G2.parameters())
    ...
    writer = SummaryWriter('runs/DIST_COMB_per_scene_lr0_002_w_mah')
    ...
    for epoch in range(EPOCHS):
        ...
        for sample, sample_data in tqdm(all_results.items()):
            ...
            mot_trackers = {tracking_name: AB3DMOT(tracking_name=tracking_name, state=epoch, COMB=COMB) for
            ...
            while current_sample_token != '':
                ...
                for tracking_name in NUSCENES_TRACKING_NAMES:
                    if dets_all[tracking_name]['dets'].shape[0] > 0:
                        ...
                        trackers, D, K = mot_trackers[tracking_name].update(dets_all[tracking_name])

                        if epoch < 10:

                            if D.numel() == 0 or K is None:  # could this be a problem??
                                continue
                            ...
                            # optimizer.zero_grad()
                            # loss.backward()
                            # max_norm = 2.0  # Set the maximum gradient norm
                            # clip_grad_norm_(params_to_optimize, max_norm)
                            # optimizer.step()
