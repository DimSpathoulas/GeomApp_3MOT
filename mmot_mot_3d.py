...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        bce_loss = -targets * \
            torch.log(inputs) - (1 - targets) * torch.log(1 - inputs)
...
class TrackerNN(nn.Module):
    ...
    def initialize_G3(self):
        ...
        for layer in self.G3:
            if isinstance(layer, nn.Conv2d):
                # Xavier normal initialization with small gain
                nn.init.xavier_normal_(layer.weight, gain=0.1)
                if layer.bias is not None:  # Ensure bias initialization
                    # Small constant value for biases to avoid zeros
    ...
    def feature_fusion(self, F2D, F3D, cam_onehot_vector):
        ...
        lidar_feats = torch.nn.functional.normalize(
            F3D.view(F3D.shape[0], -1), p=2, dim=1)
        lidar_feats = lidar_feats.view(
            lidar_feats.shape[0], F3D.shape[1], F3D.shape[2], F3D.shape[3])

        F2D_normalized = torch.nn.functional.normalize(F2D, p=2, dim=1)
        cam_feats = torch.cat((F2D_normalized, cam_onehot_vector), dim=1)
        cam_feats = self.G1(cam_feats)

        cam_feats = torch.nn.functional.normalize(
            cam_feats.view(cam_feats.shape[0], -1), p=2, dim=1)
        cam_feats = cam_feats.view(
            cam_feats.shape[0], F3D.shape[1], F3D.shape[2], F3D.shape[3])

        # Fuse the normalized features
        fused = cam_feats * 0.25 + 0.75 * lidar_feats
        # fused = self.inter(fused)
        # normalize the fused output again
        fused = torch.nn.functional.normalize(
            fused.view(fused.shape[0], -1), p=2, dim=1)
        fused = fused.view(
            fused.shape[0], F3D.shape[1], F3D.shape[2], F3D.shape[3])

        # fused = torch.randn(fused.shape[0], F3D.shape[1], F3D.shape[2], F3D.shape[3]).to(device)
    ...
    def hungarian_matching(self, estims, trues):
        ...
        cost_matrix = np.linalg.norm(
            estims_array[:, np.newaxis] - trues_array, axis=2)
    ...
    def construct_K_matrix(self, distance_matrix, dets, curr_gts, trks, prev_gts, threshold=2, epoch=0):
        ...
        dist_1 = np.linalg.norm(
            dets_array[d_idx] - curr_gts_array[d_gt_idx], axis=1)
        dist_2 = np.linalg.norm(
            trks_array[t_idx] - prev_gts_array[t_gt_idx], axis=1)
    ...
    def Criterion(self, distance_matrix=None, K=None, mask=None):
        ...
        T, C_contr, C_pos, C_neg = map(lambda x: torch.tensor(
            x, device=device), [11.0, 6.0, 3.0, 3.0])
        ...
        if pos or neg:
            ...
            # / len(neg) / len(pos)
            L_pos = ((torch.clamp(C_pos - (T - pos_distances), min=0).sum()
                      ) / len(neg)) if pos else torch.tensor(0., device=device)
            L_neg = ((torch.clamp(C_neg - (neg_distances - T), min=0).sum()
                      ) / len(pos)) if neg else torch.tensor(0., device=device)
    ...
    # FOR EACH SAMPLE IN EACH SCENE FORWARD BASED ON self.state
    def forward(self, dets_all, tracking_name):
        ...
        # LOAD FEATURES
        F2D = torch.tensor(feats, dtype=torch.float32,
                           requires_grad=True).to(device)
        F3D = torch.tensor(pcbs, dtype=torch.float32,
                           requires_grad=True).to(device)
        cam_onehot_vector = torch.tensor(
            cam_vecs, dtype=torch.float32, requires_grad=True).to(device)
        ...
        if tracking_state['features']:
            trks_feats = torch.stack(
                [feat for feat in tracking_state['features']], dim=0)
        else:
        ...
        # print(D_mah, D_feat)
        # D = 1.0 - self.compute_pairwise_cosine_similarity(det_feats=det_feats, trk_feats=trks_feats).detach().cpu().numpy()
        # print(cos_met)
        ...
        if self.state >= 1:
            with torch.no_grad():
                D_feat = self.distance_combination_stage_1(
                    feature_map).detach()
                # print(D_feat, D_mah)
        ...
        if self.training == True and self.state == 1:
            ...
            if K.shape[0] > 0:
                ...
                # criterion is costum loss
                loss = self.Criterion(D_module, K, mask)
                # print(loss)
                D = K.detach().cpu().numpy()
            else:
                # D = mah_dist
        ...
        # GREEDY MATCH
        # K, mask = self.construct_K_matrix(distance_matrix=mah_dist, dets=dets, curr_gts=curr_gts, trks=trks,
        #                                 prev_gts=prev_gts, epoch=self.epoch)
        # if K.shape[0] > 0:
        #     # print(cos_met, K, mask)
        #     D = K.detach().cpu().numpy()
        ...
        for t, trk in enumerate(tracking_state['trackers']):
            if t not in unmatched_trks:
                ...
                blended_feature = det_feats[d].detach(
                ) * self.blender + trks_feats[t] * (1.0 - self.blender)
                ...
                for idx in unmatched_dets:
                    if P[idx] > tracking_state['track_init_thresh']:
                        new_track = KalmanBoxTracker(
                            dets[idx], info[idx], info[idx, -1], tracking_name)
                        tracking_state['trackers'].append(new_track)
                        tracking_state['features'].append(
                            det_feats[idx].detach())
        ...
        # meaning if val
            ...
            for i in unmatched_dets:  # a scalar of index
                ...
                trk = KalmanBoxTracker(
                    dets[i, :], info[i, :], track_score, tracking_name)
        ...
        for i, trk in reversed(list(enumerate(tracking_state['trackers']))):
            ...
            if (trk.time_since_update < tracking_state['max_age'] and
                ...
                ret.append(np.concatenate(
                    (d, [trk.id + 1], trk.info[:-1], [trk.track_score])).reshape(1, -1))
