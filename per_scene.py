...
from Nets.net import Feature_Fusion, Distance_Combination_Stage_1
...
class AB3DMOT(object):
    ...
    def update(self, dets_all):
        ...
        # print('\n', self.tracking_name, '\n', dets.shape, '\ntrks', trks.shape)
        det_feats = self.FF(F2D, F3D, cam_onehot_vector)
...
def track_nuscenes():
    ...
    parser.add_argument('--output_path', type=str, default='per_scene_lr01_epoch20.json',
    ...
    FF = Feature_Fusion().to(device)
    DCS1 = Distance_Combination_Stage_1().to(device)
    optimizer = torch.optim.Adam(list(FF.parameters()) + list(DCS1.parameters()), lr=0.0000001)
    ...
    writer = SummaryWriter('runs/nothing')
    leds = 0
    for epoch in range(EPOCHS):

        epic_loss = torch.tensor(0.0).to(device=device)
        ...
        for sample, sample_data in tqdm(all_results.items()):
            ...
            while current_sample_token != '':
                ...
                for i, item in enumerate(all_results[current_sample_token]):
                    for name in NUSCENES_TRACKING_NAMES:
                        ...
                        # print(name, current_sample_token)
                        # print(len(dets[name]), '\n')
                ...
                for tracking_name in NUSCENES_TRACKING_NAMES:
                    if dets_all[tracking_name]['dets'].shape[0] > 0:
                        ...
                        if epoch < 10:
                            ...
                            if D is None: # will this create instability?
                                continue

                            if K is None:  # FALSE DETECTION TREATED AS INCORRECT TRACKING
                                K = torch.ones((D.shape[0], D.shape[1]), device=device, dtype=D.dtype)
            ...
            if DS:
                ...
                for Dls, Kls in zip(DS, KS):
                    for D, K in zip(Dls, Kls):
                        loss = criterion(D, K)
                ...
                epic_loss = epic_loss + total_loss
        ...
        writer.add_scalar('Loss/total', epic_loss.item(), epoch)
